#!/usr/bin/env python3
"""Import AI-generated exam Markdown into a TauriExam SQLite question bank.

This script is intentionally self-contained inside the skill folder so the skill
can be migrated as a package. It parses the shared TauriExam Markdown contract,
including drag_drop structures, and writes the same SQLite schema consumed by
TauriExam.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import AI-generated Markdown questions into SQLite.")
    parser.add_argument("--input", required=True, help="Folder containing generated Markdown files.")
    parser.add_argument("--db", default=None, help="SQLite output path. Defaults to question-banks/<EXAM>.sqlite.")
    parser.add_argument("--exam", default=None, help="Exam code. Defaults to input folder name.")
...
def normalize_question_type(value: str | None) -> str:
    normalized = (value or "single_choice").strip().lower().replace(" ", "_").replace("-", "_")
    ...
    return normalized or "single_choice"
...
def parse_batch(path: Path, exam: str, global_offset: int) -> ParsedBatch:
    ...
    for index, match in enumerate(matches):
        ...
        question = ParsedQuestion(
            ...
            source_answer=sections.get("source answer", sections.get("correct answer", sections.get("正确答案", ""))),
            recommended_answer=sections.get("my recommended answer", sections.get("correct answer", sections.get("正确答案", ""))),
            ...
            notes="\n".join(part for part in [sections.get("notes", ""), f"difficulty: {metas.get('difficulty')}" if metas.get("difficulty") else ""] if part),
            question_md=block,
            md_file=path.name,
            pdf_file=None,
            batch_id=path.stem,
    ...
    return ParsedBatch(path.stem, path.name, path.stem, None, None, None, None, None, "", questions)
...
def main() -> None:
    ...
    exam = (args.exam or root.name).upper()
    ...
    offset = 0
    for path in files:
        batch = parse_batch(path, exam, offset)
